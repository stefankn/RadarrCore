...
//  
//
//  Created by Stefan Klein Nulent on 03/08/2019.
//

import Foundation

public final class RCCalendar {
    ...
    public init(initialDate: Date = Date(), calendar: Calendar = .current) {
    ...
    public func monthCalendar(completion: @escaping (Result<[RCCalendarDate], Error>) -> Void) {
        guard let server = server, let startDate = inclusiveStartDate, let endDate = inclusiveEndDate else { return }
        
        RCService(server: server)?.send(RCGetCalendar(startDate: startDate, endDate: endDate)) { [weak self] result in
            switch result {
    ...
    private func generateCalendarMonth(for calendarDates: RCCalendarDates) -> [RCCalendarDate] {
        guard
        ...
        var dates = Set(dateRange(from: inclusiveStartDate, to: inclusiveEndDate).map{
            RCCalendarDate(date: $0, isInCurrentMonth: (startDate...endDate).contains($0))
        })
        
        calendarDates.dates.filter{ dates.contains($0) }.forEach{
            dates.update(with: RCCalendarDate(date: $0.date, movies: $0.movies, isInCurrentMonth: (startDate...endDate).contains($0.date)))
        }
